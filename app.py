...
def normalise_record(record: Dict[str, Any], index: int) -> Dict[str, Any]:
    true_labels = parse_json_like(
        record.get("true_labels", record.get("doc_labels", [])), []
    )
    predicted_labels = parse_json_like(
        record.get("predicted_labels", record.get("pred_labels", [])), []
    )
    probabilities = parse_json_like(
        record.get("label_probabilities", record.get("pred_probs", {})), {}
    )
    true_spans = parse_json_like(record.get("true_spans", []), [])
    predicted_spans = parse_json_like(
        record.get("predicted_spans", record.get("pred_spans", [])), []
    )
